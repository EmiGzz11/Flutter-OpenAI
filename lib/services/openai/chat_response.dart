...
class ChatResponse {
  ...
  factory ChatResponse.fromResponse(Response response) {
    ...
    Map<String, dynamic> parseBody = json.decode(responseBody);
    return ChatResponse(
      id: parseBody['id'],
      object: parseBody['object'],
      created: parseBody['created'],
      model: parseBody['model'],
      choices: List<Choice>.from(
          parseBody['choices'].map((choice) => Choice.fromJson(choice))),
      usage: Usage.fromJson(parseBody['usage']),
    );
  }
}
